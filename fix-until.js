(function(){
	 ...
	 * @return {void}
	 */
	Fixuntil.prototype.setElements = function(elements){
	 ...
	 * @return {void}
	 */
	Fixuntil.prototype.setTargets = function(targets){
	 ...
	 */
	Fixuntil.prototype.validateSettings = function(els, targs){
		
		var isValid = true,
		i = 0;

		// If not every element have its target
		if( els.length != targs.length ){
			console.error('Fixuntil - Invalid settings.\nMake sure every element have a respective target.');
			isValid = false;
			return isValid;
		}
		...
		for( i; i < els.length; i++ ){
			...
			// If the element and the respective target are the same	
			if( els[i] === targs[i] ){
				console.error('Fixuntil - Invalid settings.\nAn element can\'t be its own target.' );	
				isValid = false;
				return isValid;
			}

			// If the arrays content are not valid
			if( ( !(els[i] instanceof Element) && !(els[i] instanceof Array) && !(document.querySelector(els[i]) instanceof Element) ) ||
				( !(targs[i] instanceof Element) && !(targs[i] instanceof Array) && !(document.querySelector(targs[i]) instanceof Element) ) ){
				console.error('Fixuntil - Invalid settings.\nThe elements and targets arrays must contain valid query selectors or actual Elements.' );	
				isValid = false;
				return isValid;	
			}
		}
	...
	Fixuntil.prototype.init = function(){
		...
		// Uses the selectors to create Element arrays if the array objects 
		// are not Element type objects
		...
		// If some of the validations failed, stops the program
		if( !this.validateSettings(els,targs) ){
			return;
		}
		...
		for( i; i < self.getElements().length; i++ ){
			...
			(function(i){
				...
				var elLeftDist = element.getBoundingClientRect().left + window.scrollX;
